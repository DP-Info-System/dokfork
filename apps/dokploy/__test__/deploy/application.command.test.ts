import * as adminService from "@dpploy/server/services/admin";
import * as applicationService from "@dpploy/server/services/application";
import { deployApplication } from "@dpploy/server/services/application";
import * as deploymentService from "@dpploy/server/services/deployment";
import * as builders from "@dpploy/server/utils/builders";
import * as notifications from "@dpploy/server/utils/notifications/build-success";
import * as execProcess from "@dpploy/server/utils/process/execAsync";
import * as gitProvider from "@dpploy/server/utils/providers/git";
...
vi.mock("@dpploy/server/services/application", async () => {
	const actual = await vi.importActual<
		typeof import("@dpploy/server/services/application")
	>("@dpploy/server/services/application");
	return {
		...actual,
...
vi.mock("@dpploy/server/utils/providers/git", async () => {
	const actual = await vi.importActual<
		typeof import("@dpploy/server/utils/providers/git")
	>("@dpploy/server/utils/providers/git");
	return {
		...actual,
...
vi.mock("@dpploy/server/utils/process/execAsync", () => ({
	execAsync: vi.fn(),
	ExecError: class ExecError extends Error {},
}));

vi.mock("@dpploy/server/utils/builders", async () => {
	const actual = await vi.importActual<
		typeof import("@dpploy/server/utils/builders")
	>("@dpploy/server/utils/builders");
	return {
		...actual,
...
vi.mock("@dpploy/server/utils/notifications/build-success", () => ({
	sendBuildSuccessNotifications: vi.fn(),
}));

vi.mock("@dpploy/server/utils/notifications/build-error", () => ({
	sendBuildErrorNotifications: vi.fn(),
}));

vi.mock("@dpploy/server/services/rollbacks", () => ({
	createRollback: vi.fn(),
}));

import { db } from "@dpploy/server/db";
import { cloneGitRepository } from "@dpploy/server/utils/providers/git";
