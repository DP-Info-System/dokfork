import { parseRawConfig, processLogs } from "@dpploy/server";
...
`;
	...
	it("should filter out DPPloy dashboard requests", () => {
		const dpployDashboardEntry = `{"ClientAddr":"172.71.187.131:9485","ClientHost":"172.71.187.131","ClientPort":"9485","ClientUsername":"-","DownstreamContentSize":14550,"DownstreamStatus":200,"Duration":57681682,"OriginContentSize":14550,"OriginDuration":57612242,"OriginStatus":200,"Overhead":69440,"RequestAddr":"hostinger.dpploy.com","RequestContentSize":0,"RequestCount":20142,"RequestHost":"hostinger.dpploy.com","RequestMethod":"GET","RequestPath":"/_next/data/cb_zzI4Rp9G7Q7djrFKh0/en/dashboard/traefik.json","RequestPort":"-","RequestProtocol":"HTTP/2.0","RequestScheme":"https","RetryAttempts":0,"RouterName":"dpploy-router-app-secure@file","ServiceAddr":"dpploy:3000","ServiceName":"dpploy-service-app@file","ServiceURL":"http://dpploy:3000","StartLocal":"2025-12-10T05:10:41.957755949Z","StartUTC":"2025-12-10T05:10:41.957755949Z","TLSCipher":"TLS_AES_128_GCM_SHA256","TLSVersion":"1.3","entryPointName":"websecure","level":"info","msg":"","time":"2025-12-10T05:10:42Z"}`;

		// Test with only DPPloy dashboard entry - should be filtered out
		const resultOnlyDPPloy = parseRawConfig(dpployDashboardEntry);
		expect(resultOnlyDPPloy.data).toHaveLength(0);
		expect(resultOnlyDPPloy.totalCount).toBe(0);

		// Test with mixed entries - DPPloy should be filtered, others should remain
		const mixedEntries = `${dpployDashboardEntry}\n${sampleLogEntry}`;
