...
vi.mock("@dpploy/server/constants", async (importOriginal) => {
	const actual =
		await importOriginal<typeof import("@dpploy/server/constants")>();
	return {
		...actual,
		paths: () => ({
			...actual.paths(),
...
const { readValidDirectory } = await import("@dpploy/server");
