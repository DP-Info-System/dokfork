...
export const DPPLOY_DOCKER_API_VERSION =
	process.env.DPPLOY_DOCKER_API_VERSION;
export const DPPLOY_DOCKER_HOST = process.env.DPPLOY_DOCKER_HOST;
export const DPPLOY_DOCKER_PORT = process.env.DPPLOY_DOCKER_PORT
	? Number(process.env.DPPLOY_DOCKER_PORT)
	: undefined;
...
const getDockerConfig = (): Docker => {
	const versionOption = DPPLOY_DOCKER_API_VERSION
		? { version: DPPLOY_DOCKER_API_VERSION }
		: {};

	// Explicit remote Docker host configuration
	if (DPPLOY_DOCKER_HOST) {
		console.info(
			`Using remote Docker host: ${DPPLOY_DOCKER_HOST}${DPPLOY_DOCKER_PORT ? `:${DPPLOY_DOCKER_PORT}` : ""}`,
		);
		return new Docker({
			host: DPPLOY_DOCKER_HOST,
			...(DPPLOY_DOCKER_PORT && { port: DPPLOY_DOCKER_PORT }),
			...versionOption,
		});
	}
