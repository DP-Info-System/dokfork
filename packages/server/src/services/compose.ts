...
import { getBuildComposeCommand } from "@dpploy/server/utils/builders/compose";
import { randomizeSpecificationFile } from "@dpploy/server/utils/docker/compose";
import {
	cloneCompose,
	loadDockerCompose,
	loadDockerComposeRemote,
} from "@dpploy/server/utils/docker/domain";
import type { ComposeSpecification } from "@dpploy/server/utils/docker/types";
...
import { getCreateComposeFileCommand } from "@dpploy/server/utils/providers/raw";
...
export const deployCompose = async ({
	...
	const buildLink = `${await getDPPloyUrl()}/dashboard/project/${
...
export const removeCompose = async (
	...
	try {
		...
		if (compose.composeType === "stack") {
			const command = `
			docker network disconnect ${compose.appName} dpploy-traefik;
