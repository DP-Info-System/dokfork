import { findServerById } from "@dpploy/server/services/server";
import { getWebServerSettings } from "@dpploy/server/services/web-server-settings";
import type { ContainerCreateOptions } from "dockerode";
import { IS_CLOUD } from "../constants";
import { getDPPloyImageTag } from "../services/settings";
...
export const setupMonitoring = async (serverId: string) => {
	...
	const containerName = "dpploy-monitoring";
	let imageName = "dpploy/monitoring:latest";

	if (
		(getDPPloyImageTag() !== "latest" ||
			process.env.NODE_ENV === "development") &&
		!IS_CLOUD
	) {
		imageName = "dpploy/monitoring:canary";
	}
	...
	const settings: ContainerCreateOptions = {
		...
		HostConfig: {
			...
			Binds: [
				...
				"/proc:/host/proc:ro",
				"/etc/dpploy/monitoring/monitoring.db:/app/monitoring.db",
			],
			NetworkMode: "host",
		},
		ExposedPorts: {
	...
	try {
		await execAsyncRemote(
			serverId,
			"mkdir -p /etc/dpploy/monitoring && touch /etc/dpploy/monitoring/monitoring.db",
		);
		if (serverId) {
