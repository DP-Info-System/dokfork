...
export const restoreWebServerBackup = async (
	...
	try {
		...
		const tempDir = await mkdtemp(join(tmpdir(), "dpploy-restore-"));
		...
		try {
			...
			const { stdout: postgresContainer } = await execAsync(
				`docker ps --filter "name=dpploy-postgres" --filter "status=running" -q | head -n 1`,
			);

			if (!postgresContainer) {
				throw new Error("DPPloy Postgres container not found");
			}
			...
			await execAsync(
				`docker exec ${postgresContainerId} psql -U dpploy postgres -c "SELECT pg_terminate_backend(pg_stat_activity.pid) FROM pg_stat_activity WHERE pg_stat_activity.datname = 'dpploy' AND pid <> pg_backend_pid();"`,
			);

			emit("Dropping existing database...");
			await execAsync(
				`docker exec ${postgresContainerId} psql -U dpploy postgres -c "DROP DATABASE IF EXISTS dpploy;"`,
			);

			emit("Creating fresh database...");
			await execAsync(
				`docker exec ${postgresContainerId} psql -U dpploy postgres -c "CREATE DATABASE dpploy;"`,
			);
			...
			await execAsync(
				`docker exec ${postgresContainerId} pg_restore -v -U dpploy -d dpploy /tmp/database.sql`,
			);
