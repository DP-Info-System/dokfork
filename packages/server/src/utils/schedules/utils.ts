...
import type { Schedule } from "@dpploy/server/db/schema/schedule";
import {
	createDeploymentSchedule,
	updateDeployment,
	updateDeploymentStatus,
} from "@dpploy/server/services/deployment";
import { findScheduleById } from "@dpploy/server/services/schedule";
...
export const runCommand = async (scheduleId: string) => {
	...
	} else if (scheduleType === "dpploy-server") {
		try {
			const writeStream = createWriteStream(deployment.logPath, { flags: "a" });
